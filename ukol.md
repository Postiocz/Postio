...
- [x] **Krok 2 – Tlačítko „Označit jako publikované":** Přidat do (a) widgetu „K vyřízení" na `dashboard/page.tsx` (karta `todoPosts`, vedle Kopírovat/Stáhnout) a (b) karty příspěvku `_post-card.tsx` (pouze když příspěvek/platforma má stav `'ready'` pro twitter). Tlačítko volá server action z Kroku 3.
- [x] **Krok 3 – Server Action `markAsPublishedManual`:** V `src/lib/actions/publish.ts` nová `"use server"` funkce. Změní `post_platforms.status` z `'ready'` na `'published'` a nastaví `published_at = now()` (scope dle `account_id`/`platform`, jako ostatní handlery). Revalidace jako v `handlePublishSuccess`.
- [x] **Krok 4 – UI text + i18n:** V editoru `posts/new/page.tsx` změnit text hlavního tlačítka publikování pro manuální X účty na „Připravit k vyřízení 🔔". Doplnit překlady `dashboard.markPublished` / `dashboard.markPublishedToast` / `posts.markPublished` (cs/en/uk) a použít v Kroku 2.
- [x] **Krok 5 – Indikátor „K vyřízení" v seznamu i kalendáři:** Pro platformy ve stavu `post_platforms.status === 'ready'` (manuální X) přidat viditelný odznáček/badge (sky/indigo) do (a) karty příspěvku `_post-card.tsx` a (b) kalendářního chipu. Uživatel tak uvidí, že post se ještě musí ručně publikovat na X. Dle Pravidla 8 použít design-manuály.
