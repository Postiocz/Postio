...
- [x] **KROK 3: Ochrana proti smazání** — ✅ hotovo
      ...
      - UI: koš dostupný jen pro custom plány; `!plan.is_master_template` pojistka.
      Otestováno – u master šablony koš chybí, u custom fungue. Celý Prompt054 hotán.

### 10.5 Prompt 058 – Granulární viditelnost (Visibility Rules)

**Problém:** Logika `is_new_user_only` je příliš hrubá. Admin potřebuje přesně
definovat, komu se který plán zobrazí (kombinace skupin uživatelů), ne jen
"nový uživatel yes/no".

**Cíl:** Nahradit flagy systémem cílených skupin (`visibility_rules` = TEXT[]),
které plně řídí zobrazení i nákup v app.

Skupiny (`visibility_rules`): `['anonymous']` | `['free']` | `['creator']` | `['pro']`
+ libovolné kombinace (např. `['anonymous','free']`).

- [x] **KROK 1: SQL Migrace (visibility_rules)** — ✅ hotovo
      Migrace `051_visibility_rules.sql`: `visibility_rules TEXT[] DEFAULT '{}'` +
      backfill (master→vše, promo→`['anonymous']`, ostatní→`['anonymous']`) + GIN index.
      `types.ts` aktualizováno (Row/Insert/Update).

- [x] **KROK 2: Admin UI update + Server Action** — ✅ hotovo
      Checkboxy „Kdo uvidí tento plán?" (anonymous/free/creator/pro) v editačním
      i create modálu; payloady `updatePricingPlan`/`createPricingPlan` ukládají
      `visibility_rules`. Překlady cs/en/uk.

- [x] **KROK 3: Globální filtr zobrazení** — ✅ hotovo
      `pricing-section.tsx`: session-aware (přihlášený→jeho tarif, nepřihlášený→anonymous).
      `billing/page.tsx`: `.contains(visibility_rules,[userPlan])`. VÝJIMKA: aktuální
      instance vždy viditelná. Nav na landing page pozná přihlášeného uživatele.

- [x] **KROK 4: Ochrana v Checkout API** — ✅ hotovo
      `/api/stripe/checkout` ověřuje `visibility_rules` (master i custom) → 403,
      kromě výjimky aktuální instance. Promo plány dostaly `['anonymous','free']`,
      aby nově registrovaný free uživatel dokončil nákup.

- [x] **KROK 5: Smart Pricing Cards (Frontend UX)** — ✅ hotovo
      `pricing-client.tsx`: přihlášený → přímý checkout (spinner, bez login redirectu);
      odhlášený → cookie + login. Oprava: měna checkoutu se řídí měnou aktivního
      předplatného (Stripe nedovolí kombinovat měny u jednoho zákazníka).
